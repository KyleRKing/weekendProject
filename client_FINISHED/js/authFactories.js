...
function authFactory($http, $q, authTokenFactory){
	...
	authFactory.index = function(){
		return $http.get('http://10.0.11.66:8080/api/users')
	}
	// handle login
	authFactory.login = function(username, password){
		return $http.post('http://10.0.11.66:8080/api/authenticate', {
			username: username,
			password: password
	...
	authFactory.signup = function(username, password){
		return $http.post('http://10.0.11.66:8080/api/users', {
			username: username,
			password: password
		})
	}
	...
	authFactory.getUser = function(){
		if(authTokenFactory.getToken()){
			return $http.get('http://10.0.11.66:8080/api/me')
		} else {
	...
	//addTask maybe?
	authFactory.addTask = function( task ) {
		if ( authTokenFactory.getToken() ) {
			console.log( "Sent task")
			console.log( task )
			return $http.post( 'http://10.0.11.66:8080/api/todo/new', task )
		} else {
			return $q.reject( { message: 'User has no token' } )
		}
	}
